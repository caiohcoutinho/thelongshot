 ...
 */
public class CorrectPlayerRotationMoveCondition implements Condition {
    @Override
    public boolean check(Stage stage, Player player, Turn turn) {
        return player.getRotation().equals(turn.getMove().getRotation());
    }

    @Override
    public ErrorMessage errorMessage() {
        return new ErrorMessage(BattleWebSocketService.PLAYER_OUT_OF_ROTATION, "Player out of rotation!");
    }
}
